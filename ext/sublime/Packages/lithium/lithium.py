...
import os, io, platform, json, re, sys, json, traceback
import webbrowser
import functools
...
settings = {
    ...
    'li_opts'       : { "verbosity" : "2",  "std": "SublimeStd", "header": "1"},
    ...
    'doc_servers' : {
        'dash': {
            ...
            "keys_map": {
              ...
              "Jade"                  : ["jade"],
              "Java"                  : ["java", "javafx", "grails", "groovy", "playjava", "spring", "cvj", "processing", "javadoc", "mydoc"],
...
def li_warn(msg):
    print(msg)
    # sublime.active_window().set_status_bar_visible(True)
...
def li_parse_output_view(view = None):
    ...
    if view is None:
        li_show_message("There is not a lithium output view")
        return
    ...
    for r in place_detectors:
        res     = []
        regions = view.find_all(r, sublime.IGNORECASE, "\\1" + delim + "\\2" + delim + "\\3", res)
        reg_idx = 0
        for p in res:
            e = p.split(delim)
            ...
            paths.append([ fn, ln, ms, regions[reg_idx] ])
            reg_idx = reg_idx + 1
...
def li_show_items(items, done = None, selected_index = 0):
    l = len(items)
    if l > 0:
        if li_is_debug():
        ...
        sublime.active_window().show_quick_panel(items, done, selected_index = selected_index)
    else:
        if li_is_debug():
...
def li_run(command, output_handler = None, error_handler = None, run_async = True, options = None):
    ...
    if options is None:
        options = dict(settings.get("li_opts")) # create a copy of the object

    if 'basedir' not in options:
        bd = li_project_home()
        if bd is None:
            li_show_message("Project base dir cannot be detected (most likely there is no .lithium folder in parent hierarchy)")
            return
        options['basedir'] = bd
...
def li_sel_region(view = None):
    if view is None:
        view = li_current_view()
...
def li_view_symbol(view, region = None):
    if view is not None:
        symb = None
        if region is None:
            region, symb = li_sel_region(view)
            if region is None:
                if li_is_debug():
...
# new java.[util].HashMap();
    ...
    for direction in [ False, True ]:
        ...
        while True:
            ...
            if ps >= 0  and view.substr(ps) == '.':
                ...
                print("Detected '%s' word scope = %s " % (view.substr(wr), view.scope_name(ws)))
    ...
    print("Detected symbol: %s" % symbol)
    ...
    if li_is_debug():
        print("java_view_symbol(): pkgs = " + str(pkg_name) + ", class_name = " + str(class_name) + ", const = " + str(const_name) + "," + symbol)
...
def java_class_info(view, symbol):
    js         = []
    is_started = []
    ...
    li_run("ShowClassInfo:%s" % symbol, output, error, False, { "std": "none" })
    return js
...
jsonStr = """
    {
        ...
        "interfaces": [ "java.util.Collection",  "java.util.Set", "java.util.ArrayList", "java.util.Arrays"],
        "fields": [
            {
                "name": "field1",
        ...
        "methods": [
...

            {
                ...
                "args" : [
                    "java.util.Date",
                    "com.test.insight.me.package.KeyStoreTypeMedia",
                    "com.test.insight.me.package.KeyStoreTypeMedia",
                    "com.test.insight.me.package.KeyStoreTypeMedia"
                ],
                "declareHere": true,
...
def filterMethods(json_methods):
    global METHODS_FILTERS
    return [ m for m in json_methods if m['level'] is None or METHODS_FILTERS.get(m['level']) == True ]


def generateMethodsContent(jsonObj):
    global METHODS_FILTERS
    global UI
    ...
    template = TEMPLATES_ENV.get_template('classInfoTemplate.html')
    #template = Template(CLASS_METHODS_TEMPLATE)

    return template.render(
        clazz = jsonObj,
        #methods = sorted(jsonObj.get('methods'), key = functools.cmp_to_key(cmp)),
        methods = sorted(jsonObj.get('methods'), key = functools.cmp_to_key(cmp), reverse=True),
        fields = sorted(jsonObj.get('fields'), key = functools.cmp_to_key(cmp), reverse=True),
        name = jsonObj.get('name'),
        filters = METHODS_FILTERS,
        ui = UI
    )

def updateMethodsFilter(f):
    global METHODS_FILTERS
    global LAST_CLASSINFO_JSONOBJ
    global UI

    prefix = f[0:f.find(':')]
    key    = f[len(prefix) + 1:len(f)]
    if prefix == "filter":
        if METHODS_FILTERS.get(key) is not None:
            METHODS_FILTERS[key] = not METHODS_FILTERS[key]
    elif prefix == "ui":
        print("KEY = " + key)
        for k in UI:
            UI[k] = False
    ...
    sublime.active_window().active_view().update_popup(generateMethodsContent(LAST_CLASSINFO_JSONOBJ))

def showMethods(view, jsonObj):
    global LAST_CLASSINFO_JSONOBJ

    LAST_CLASSINFO_JSONOBJ = jsonObj
    view.show_popup(
        generateMethodsContent(jsonObj),
        max_width = 900,
        max_height = 700,
        on_navigate = updateMethodsFilter
    )

    #view.show_popup(html, max_width = 700, max_height = 500, on_navigate = None)
...
class liShowPomCommand(sublime_plugin.WindowCommand):
    def is_enabled(self, **args):
        home = li_project_home()
        if home is not None:
            p = os.path.join(home, 'pom.xml')
            if os.path.exists(p):
                return True
        return False

    def run(self, **args):
        sublime.active_window().open_file(os.path.join(li_project_home(), 'pom.xml'))


class lithiumCommand(sublime_plugin.WindowCommand):
    def is_enabled(self, **args):
        home = li_project_home()
        if home is not None:
            p = os.path.join(home, '.lithium', 'project.rb')
            if os.path.exists(p):
                return True
        return False

    def run(self, **args):
        sublime.active_window().open_file(os.path.join(li_project_home(), '.lithium', 'project.rb'))
...
class liCommand(sublime_plugin.WindowCommand):
    ...
    def run(self, **args):
        ...
        #sublime.active_window().new_html_sheet("Test", "<h1>whejqwhjeh</h2>", sublime.ADD_TO_SELECTION, -1)

        # item = sublime.QuickPanelItem(
        #     "<li>dsdds</li>",
        #     "<li>jwkjkqwjekqjek</li><li>jwkjkqwjekqjek</li>",
        #      "sds",
        #      (sublime.KIND_ID_FUNCTION, "p", "Property")
        # )
        # sublime.active_window().show_quick_panel([
        #     [ "sjkdaksjkdjsd <li>dsdddsd</li>",
        #       "<h1>dsdsd</h1>" ],
        #     item
        # ], lambda x: print("!!!!!!!"), sublime.MONOSPACE_FONT)

        # showMethods(sublime.active_window().active_view(), jsonObj)
        # return

        # # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        # End of experimental test part
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
class liCompleteImportCommand(liJavaTextCommand):
    ...
    def run(self, edit, **args):
        ...
        self.region, self.word = li_sel_region(self.view)
    ...
    def output(self, process, line):
        ...
        if line is not None:
            ...
            if l > 20:
                li_append_output_view("(W) [SUB]  Too many variants detected:\n%s" % "\n    ".join(self.found_items))
...
class liShowClassMethodsCommand(liJavaTextCommand):
    ...
    def run(self, edit, **args):
        if "paste" in args:
            ...
            region, word = li_sel_region(self.view)
...
class liGotoClassCommand(liJavaTextCommand):
    def run(self, edit, **args):
        ...
        if view is None or view.is_auto_complete_visible():
            li_warn("View is not available")
            return

        fullclazz, clazz, cnst, word = java_view_symbol(view)
        if fullclazz is None:
            li_warn("Class name cannot be detected")
            return

        sublime.active_window().run_command("show_overlay", { "overlay": "goto", "show_files" : "true", "text": fullclazz.replace('.', '/') }  )
...
class liShowClassInfoCommand(liJavaTextCommand):
    def run(self, edit, **args):
        ...
        if word is not None and word != '':
            js = java_class_info(self.view, word)
            if len(js) > 0:
                #json_str =
                showMethods(self.view, json.loads(''.join(js)))
                #li_show_items_in_popup(self.view, word, detected_modules)
            else:
                li_show_message("No module has been discovered for %s" % word)
        else:
            li_show_message("Class name and package cannot be fetched")
...
class AnnotatedView(sublime_plugin.ViewEventListener):
    ...
    def add(self, view, msg):
        pass
        #if view.id() not in self.phantoms:



class TestMarkCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        #     icon = "bookmark",
        #     scope = "invalid"
        #     ,annotations = [ "<a href='dsd' title='dsdsdsd sdghsghd sgdhdsdgh sgdhgsgdhg ghsgdhgsdh ghsgdh gdhs'>jdsdjks</a>" ]
        #     ,annotation_color = "orange"
        #     ,flags =  sublime.DRAW_NO_FILL
        # )

        #self.view.erase_phantoms("test")
        #self.view.add_phantom("test", self.view.sel()[0], "Hello, World!", sublime.LAYOUT_BLOCK)

        ps = sublime.PhantomSet(self.view, "test2")
        ps.update([
            sublime.Phantom(self.view.sel()[0], "Hello world", sublime.LAYOUT_BLOCK)
        ])
...
class liShowLocationsCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit, **args):
        ...
        #     print("--- %s" % ll[3])
        #     #scope = "region.redish",
        #     li_current_view().add_regions("test2", [ ll[3] ], annotations=["HELLO"], icon = "dot")

        #     print(len(li_current_view().get_regions("test2")))
        ...
        if len(self.locations) > 0:
            if tp == 'next':
                ...
                li_show_items(locs, self.done, self.last_selected)
        else:
    ...
    def next(self):
        if len(self.locations) > self.last_selected + 1:
            self.last_selected = self.last_selected + 1
        else:
            self.last_selected = 0

        self.go_to_location(self.locations[self.last_selected])

    def prev(self):
        if self.last_selected > 0:
            self.last_selected = self.last_selected - 1
        else:
            self.last_selected = len(self.locations) - 1
            ...
            if len(rset) > 0:
                ...
                if p == None or len(p) == 0:
                    ...
                    if panel is not None:
                        ...
                        li_show_message("liGoToLocationCommand.run(): There is no an active view to go")
        else:
            if li_is_debug():
